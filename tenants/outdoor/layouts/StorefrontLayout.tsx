...
/**
 * Outdoor — storefront layout (header/footer/nav shell).
 ...
 * tenants/outdoor/styles/theme.css, never via tenant checks in code.
 */
export function OutdoorStorefrontLayout({
  children,
}: {
    ...
    >
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
          ...
          {outdoorConfig.nav.length > 0 && (
            <nav className="hidden items-center gap-8 md:flex">
              {outdoorConfig.nav.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
      ...
      >
        <div
          ...
          style={{ color: "var(--brand-primary)" }}
        >
          © {new Date().getFullYear()} {outdoorConfig.name}. All rights
          reserved.
        </div>
      </footer>
    </div>
